...
const SearchableCheckList = ({
  ...
  let itemsToRender = (0, _lodash.default)(eligibleItems).filter(({
    value
  }) => _lodash.default.includes(selectedItemsKeys, value)).slice(0, displayCount).value();
  ...
  if (itemsToRender.length < displayCount) {
    ...
    const unSelectedItems = _lodash.default.filter(eligibleItems, ({
      value
    }) => _lodash.default.includes(unSelectedItemsKeys, value));
  ...
  return /*#__PURE__*/_react.default.createElement("div", {
  ...
  }, _lodash.default.map(itemsToRender, ({
    value,
    label
  }) => /*#__PURE__*/_react.default.createElement(_CheckboxGroup.default.Item, {
    key: `${value}-key`,
    label: label,
    value: value,
    dts: `${value}-dts`
    ...
    className: "footer"
  }, !_lodash.default.isEmpty(footerText) ? footerText : `${remainingItemsCount.toLocaleString()} more ${_lodash.default.toLower(footerLabel)}`)));
};
...
var _default = exports.default = SearchableCheckList;