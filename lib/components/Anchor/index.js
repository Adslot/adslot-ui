...
const Anchor = props => {
  ...
  (0, _invariant.default)(href, 'Anchor: should not be used without href. Use <Button/> for onClick-only actions.');
  (0, _invariant.default)(!(round && (!icon || !_lodash.default.isEmpty(children))), 'Anchor: round can only be used with an icon and no children.');
  (0, _invariant.default)(!(icon && _lodash.default.isEmpty(children) && !rest['aria-label'] && !rest['aria-labelledby']), 'Anchor: an aria-label or aria-labelledby is required on icon anchors.');
  (0, _invariant.default)(!(isLink && (color !== 'default' || size === 'large')), 'Anchor: anchors with the "link" variant do not inherit size and color properties.');
