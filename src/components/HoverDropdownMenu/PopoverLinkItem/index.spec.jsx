...
describe('<PopoverLinkItem />', () => {
  ...
  it('should render with default props', () => {
    ...
    expect(getByText('Link 1').parentElement).toHaveAttribute('href', 'www.some.url.com');
  });
  ...
  it('should not have href props on `PopoverLinkItem` when `target` is _modal', () => {
    props.target = '_modal';
    const { getByText } = render(<PopoverLinkItem {...props} />);
    expect(getByText('Link 1').parentElement).not.toHaveAttribute('href');
  });

  it('should add `rel` to anchor props when target is "_blank"', () => {
    props.target = '_blank';
    const { getByText } = render(<PopoverLinkItem {...props} />);
    expect(getByText('Link 1').parentElement).toHaveAttribute('rel', 'noopener noreferrer');
  });
});
