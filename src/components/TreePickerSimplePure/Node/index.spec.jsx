...
it('should render metadata of nodes already selected containing ancestory data', () => {
  render(
    <TreePickerNode itemType={itemType} node={cbrNodeAlreadySelected} includeNode={jest.fn()} removeNode={jest.fn()} />
  );
...
it('should render the button first when selected is true', () => {
  render(<TreePickerNode itemType={itemType} node={cbrNode} selected includeNode={jest.fn()} removeNode={jest.fn()} />);
...
it('should accept both strings and numbers as node ids', () => {
  ...
  expect(screen.getByTestId('grid-row-wrapper')).toHaveAttribute('data-test-selector', 'example-item-type-au-act-cbr');

  view.rerender(<TreePickerNode itemType={itemType} node={maleNode} includeNode={jest.fn()} removeNode={jest.fn()} />);
