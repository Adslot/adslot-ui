...
it('should handle multi selections', () => {
  ...
  expect(screen.getAllByClass('select-component__multi-value__label')).toHaveLength(2);
  expect(screen.getByText('Australia')).toBeInTheDocument();
...
it('should render select options in body if props.isInModal is true', () => {
  ...
  expect(screen.getAllByClass('select-component__menu-portal')).toHaveLength(1);
});
