import Props from '../containers/Props.jsx';
import DesignNotes from '../containers/DesignNotes.jsx';

## Button Group

```jsx live=true
const Example = () => {
  const [isDropdownOpen, setIsDropdownOpen] = React.useState(false);

  return (
    <React.Fragment>
      <ButtonGroup color="success">
        <Button>Approve</Button>
        <DropdownMenu>
          <DropdownMenu.Trigger
            aria-label="Toggle menu"
            icon={<SvgSymbol href="./assets/svg-symbols.svg#caret-down" />}
          />
          <DropdownMenu.Content placement="bottom">
            <DropdownMenu.Item onClick={() => console.log('click')}>Option One</DropdownMenu.Item>
            <DropdownMenu.Item>Option Two</DropdownMenu.Item>
          </DropdownMenu.Content>
        </DropdownMenu>
      </ButtonGroup>
      <ButtonGroup color="danger" variant="inverse">
        <Button>Reject</Button>
        <Popover
          triggers="disabled"
          isOpen={isDropdownOpen}
          id="popover-2"
          popoverContent="I am a Overlay on click!"
          placement="bottom"
        >
          <Button
            className="svg-icon"
            aria-label="Toggle menu"
            icon={<SvgSymbol href="./assets/svg-symbols.svg#caret-down" />}
            onClick={() => setIsDropdownOpen(!isDropdownOpen)}
          />
        </Popover>
      </ButtonGroup>
      <ButtonGroup color="primary">
        <Button>Sign off</Button>
        <Button
          className="svg-icon"
          aria-label="Toggle menu"
          icon={<SvgSymbol href="./assets/svg-symbols.svg#caret-down" />}
          onClick={() => alert('>I am a Alert on click!')}
        />
      </ButtonGroup>
      <ButtonGroup variant="inverse" disabled={true}>
        <Button>Disabled</Button>
        <Button
          className="svg-icon"
          aria-label="Toggle menu"
          icon={<SvgSymbol href="./assets/svg-symbols.svg#caret-down" />}
        />
      </ButtonGroup>
    </React.Fragment>
  );
};

render(
  <div style={{ display: 'flex' }}>
    <Example />
  </div>
);
```

### Design Notes

<DesignNotes>
  <p>
    <span className="text-bold">Button Groups</span> provides a layout for a two or more buttons to share common style
    and functionality but with independent events.
  </p>
</DesignNotes>
<Props componentName="ButtonGroup" />
