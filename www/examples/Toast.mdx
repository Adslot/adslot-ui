...
```jsx live=true
const Example = () => {
  const [notification, triggerNotification] = React.useState(0);
  ...
  return (
    <React.Fragment>
      <Toast.Container />
      <Button onClick={() => triggerNotification(notification + 1)}>Show Info Toast</Button>
      {notification ? showInfoToast() : null}
    </React.Fragment>
  );
};
render(Example);
