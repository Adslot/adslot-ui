...
const RenderNodeCtx = /*#__PURE__*/React.createContext();
export const HiddenByIdCtx = /*#__PURE__*/React.createContext({});
export const useRenderNode = () => React.useContext(RenderNodeCtx);
export const useHiddenById = () => React.useContext(HiddenByIdCtx);
export const TreePickerProvider = ({
  children,
  renderNode
}) => {
  ...
  }, /*#__PURE__*/React.createElement(RenderNodeCtx.Provider, {
    value: renderNode
  }, children));
};
...
export const useInternalActions = () => {
  ...
  const backTo = React.useCallback(nodeId => {
    setState(prev => {
  ...
  const goTo = React.useCallback((nodes, node) => {
    setState(prev => {
      if (!node) {
        // init root
  ...
  const searchTo = React.useCallback((nodes, search) => {
    setState(prev => {
      if (search) {
        return {
          ...prev,
  ...
  const addAddable = React.useCallback((nodeId, addNode) => {
    setState(prev => {
  ...
  const removeAddable = React.useCallback(nodeId => {
    setState(prev => {
  ...
  const setIsResolvingRoot = React.useCallback(newValue => {
    setState(prev => ({
      ...prev,
      isResolvingRoot: newValue
    }));
  }, [setState]);
  const setExpandingNodeId = React.useCallback((nodeId, expanding) => {
    setState(prev => ({
      ...prev,
