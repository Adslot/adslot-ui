...
const SearchableCheckList = ({
  ...
  let itemsToRender = _(eligibleItems).filter(({
    value
  }) => _.includes(selectedItemsKeys, value)).slice(0, displayCount).value();
  ...
  if (itemsToRender.length < displayCount) {
    ...
    const unSelectedItems = _.filter(eligibleItems, ({
      value
    }) => _.includes(unSelectedItemsKeys, value));
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    value,
    label
  }) => /*#__PURE__*/React.createElement(CheckboxGroup.Item, {
    key: `${value}-key`,
    label: label,
    value: value,
    dts: `${value}-dts`
